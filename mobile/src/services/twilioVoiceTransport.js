...
let twilioVoiceModule = null;

const getTwilioVoiceModule = () => {
  if (!liveCallsEnabled) {
    return null;
  }

  if (!twilioVoiceModule) {
    twilioVoiceModule = require('@twilio/voice-react-native-sdk');
  }
...
const getVoiceInstance = () => {
  ...
  if (!voiceInstance) {
    const module = getTwilioVoiceModule();

    if (!module?.Voice) {
      throw new Error('Live calls are not available in this build.');
    }

    voiceInstance = new module.Voice();
  }
...
const ensureAudioDeviceEventsBound = () => {
  ...
  const sdkModule = getTwilioVoiceModule();
  if (sdkModule?.Voice) {
    voice.addListener(sdkModule.Voice.Event.AudioDevicesUpdated, handleAudioDevicesUpdated);
...
export const startTwilioVoiceCall = async ({ token, params = {}, onStatusChange, onError, onTrace }) => {
  if (!liveCallsEnabled) {
    return {
      success: false,
      error: 'In-app live calls are not available in this build.'
    };
  }
  ...
  try {
    ...
    const voice = getVoiceInstance();
    const { Call } = getTwilioVoiceModule();
    ensureAudioDeviceEventsBound();
    ...
    connectTimeoutId = setTimeout(async () => {
      ...
      const timeoutMessage = Platform.OS === 'ios'
        ? 'The iPhone call stayed in connecting for too long. The voice provider never confirmed a live connection.'
        : 'The call stayed in connecting for too long. The voice provider never confirmed a live connection.';
    ...
    call.on(Call.Event.ConnectFailure, (error) => {
      ...
      const failureMessage = [
        ...
        Platform.OS === 'ios' ? 'iPhone call setup did not complete through the current voice provider.' : null
      ].filter(Boolean).join('\n');
...
export const endTwilioVoiceCall = async () => {
  if (!activeCall) {
    updateAudioDeviceState({ audioDevices: [], selectedDevice: null });
    updateMuteState(false);
    return {
...
export const getTwilioVoiceCallActive = () => Boolean(activeCall);

export const getTwilioAudioDeviceState = () => audioDeviceState;

export const getTwilioMuteState = () => muteState;